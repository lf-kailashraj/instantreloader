...
public class AsmInjector {
	...
	public static void clinitFieldInit(MethodVisitor mv,
		...
		mv.visitMethodInsn(Opcodes.INVOKESPECIAL,
				Type.getInternalName(FieldsHolder.class), "<init>",
		...
		mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, Type
				.getInternalName(ClassReloaderManager.class),
				"getClassReloader", Type.getMethodDescriptor(
						Type.getType(ClassReloader.class), Type.LONG_TYPE));
		mv.visitFieldInsn(Opcodes.PUTSTATIC, ownerClassInternalName,
	...
	public static void beforeMethodCheck(MethodVisitor mv, int methodAccess,
		...
		ga.visitFieldInsn(Opcodes.GETSTATIC, ownerClassInternalName,
				Constants.OUTLIER_CLASS_RELOADER_FIELDS,
				Type.getDescriptor(ClassReloader.class));
		ga.visitMethodInsn(Opcodes.INVOKEVIRTUAL,
				Type.getInternalName(ClassReloader.class), "checkAndReload",
		...
		ga.loadArgs();
		ga.visitMethodInsn(
				Modifier.isStatic(methodAccess) ? Opcodes.INVOKESTATIC
						: Opcodes.INVOKESPECIAL, ownerClassInternalName,
				methodName, methodDesc);
		ga.returnValue();
		ga.visitLabel(label);
	}

	public static void initHotCodeInstanceFieldIfNull(MethodVisitor mv,
			String fieldOwner) {
		mv.visitFieldInsn(Opcodes.GETFIELD, fieldOwner,
		...
		mv.visitMethodInsn(Opcodes.INVOKESPECIAL,
				Type.getInternalName(FieldsHolder.class), "<init>",
				Type.getMethodDescriptor(Type.VOID_TYPE));
		mv.visitFieldInsn(Opcodes.PUTFIELD, fieldOwner,
				Constants.OUTLIER_INSTANCE_FIELDS,
				Type.getDescriptor(FieldsHolder.class));
		mv.visitLabel(label);
	}
